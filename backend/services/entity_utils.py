...
def limpiar_entidades(
    ...
    if len(respaldadas) < min_entidades:
        logger.warning(f"⚠️ Solo {len(respaldadas)} entidades respaldadas. Se devuelven todas, pero puede requerirse reintento.")
        return respaldadas if respaldadas else ["[No declarado]"]

    if len(respaldadas) > max_entidades:
        logger.info(f"⚠️ Se truncan entidades de {len(respaldadas)} a {max_entidades}.")
